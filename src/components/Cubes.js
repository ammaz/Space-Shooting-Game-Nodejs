...
export default function InstancedCubes() {
  ...
  useFrame((state, delta) => {
    ...
    if (ship.current) {
      if (
        ship.current.position.z > diamondStart ||
        ship.current.position.z < diamondEnd
      ) {
    ...
    cubes.forEach((cube, i) => {
      if (ship.current) {
        if (cube.z - ship.current.position.z > -15) {
          // No need to run the rather expensive distance function if the ship is too far away
          if (
            cube.x - ship.current.position.x > -15 ||
            cube.x - ship.current.position.x < 15
          ) {
            const distanceToShip = distance2D(
        ...
        if (cube.z - ship.current.position.z > 15) {
          if (isOutsideDiamond) {
            cube.z =
              ship.current.position.z - PLANE_SIZE + randomInRange(-200, 0);
            cube.y = -CUBE_SIZE;
