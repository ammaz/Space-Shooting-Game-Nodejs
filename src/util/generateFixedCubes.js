  ...
  const wallCoordinates = [...Array(segments)].map((cube, index) => {
    return {
      ...
      z:
        index <= segments / 2
          ? -(index * CUBE_SIZE)
          : -(segments * CUBE_SIZE) + index * CUBE_SIZE, // pyramid formation
    };
  });
...
function makeDiamond(size = 21, tunnelLength = 10) {
    ...
    return {
      x:
        index === 0
          ...
          : -CUBE_SIZE * 3 - size * CUBE_SIZE + index * CUBE_SIZE,
      y: CUBE_SIZE / 2,
  ...
  const secondDiamond = firstDiamond.map((coordinates, index) => ({
    ...coordinates,
    z:
      index >= 42
        ? coordinates.z - 700 /* 735 */
        : coordinates.z - size * CUBE_SIZE * 1.75,
  }));
